...
def _cert_is_up_to_date(cert_path: str, mac: str, host_ip: str) -> bool:
    """True if the existing cert.pem already carries a SAN matching host_ip,
    an ExtendedKeyUsage(serverAuth) extension, AND a CN matching the current
    `mac` — used to auto-regenerate certs from before either fix, or after
    the user corrects a wrong auto-detected/configured MAC (see the `mac`
    add-on option — diyHue's own docs call out that a wrong MAC here
    generates a cert the official Hue app won't accept), without requiring
    the user to manually delete /data/cert.pem. Bump what this checks
    whenever ensure_certificate() gains another extension that existing
    installs need to pick up."""
    try:
        with open(cert_path, "rb") as fp:
            cert = x509.load_pem_x509_certificate(fp.read())
        cn_attrs = cert.subject.get_attributes_for_oid(NameOID.COMMON_NAME)
        if not cn_attrs or cn_attrs[0].value != mac:
            return False
...
def ensure_certificate(config_dir: str, mac: str, host_ip: str) -> str:
    ...
    if os.path.isfile(cert_path):
        if _cert_is_up_to_date(cert_path, mac, host_ip):
            return cert_path
        logging.info("Existing certificate is outdated for mac=%s host_ip=%s — regenerating", mac, host_ip)
    else:
